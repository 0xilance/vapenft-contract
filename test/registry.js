...
const MockERC1155PresetMinterPauser = artifacts.require(
  "MockERC1155PresetMinterPauser"
);
...
contract("Registry", function ([...accounts]) {
  ...
  describe("Registry", function () {
    ...
    beforeEach(async function () {
      ...
      mockFoundationTreasury = await MockFoundationTreasury.new({
        from: foundationDeployer,
      });
      ...
      mockERC1155PresetMinterPauser = await MockERC1155PresetMinterPauser.new({
        from: erc1155PresetDeployer,
      });
    });

    it("override test", async function () {
      await truffleAssert.reverts(
        registry.setRoyaltyLookupAddress(owner, mockContract.address),
        "Invalid input"
      );
      await truffleAssert.reverts(
        registry.setRoyaltyLookupAddress(mockContract.address, owner),
        "Invalid input"
      );
      await truffleAssert.reverts(
        registry.setRoyaltyLookupAddress(
          mockContract.address,
          mockManifold.address,
    ...
    it("permissions test", async function () {
      assert.equal(
        false,
        await registry.overrideAllowed(mockContract.address, { from: random })
      );
      assert.equal(
        false,
        await registry.overrideAllowed(mockManifold.address, { from: random })
      );
      await mockFoundation.setFoundationTreasury(mockContract.address);
      assert.equal(
        false,
        await registry.overrideAllowed(mockFoundation.address, { from: random })
      );
      assert.equal(
        false,
        await registry.overrideAllowed(mockRaribleV1.address, { from: random })
      );
      assert.equal(
        false,
        await registry.overrideAllowed(mockRaribleV2.address, { from: random })
      );
      assert.equal(
        false,
        await registry.overrideAllowed(mockEIP2981.address, { from: random })
      );
      assert.equal(
        false,
        await registry.overrideAllowed(mockNiftyBuilder.address, {
          from: random,
        })
      );
      assert.equal(
        false,
        await registry.overrideAllowed(mockERC1155PresetMinterPauser.address, {
          from: random,
        })
      );
      ...
      assert.equal(
        true,
        await registry.overrideAllowed(mockManifold.address, {
          from: manifoldDeployer,
        })
      );
      assert.equal(
        true,
        await registry.overrideAllowed(mockFoundation.address, {
          from: foundationDeployer,
        })
      );
      assert.equal(
        true,
        await registry.overrideAllowed(mockRaribleV1.address, {
          from: raribleV1Deployer,
        })
      );
      assert.equal(
        true,
        await registry.overrideAllowed(mockRaribleV2.address, {
          from: raribleV2Deployer,
        })
      );
      assert.equal(
        true,
        await registry.overrideAllowed(mockEIP2981.address, {
          from: eip2981Deployer,
        })
      );
      assert.equal(
    ...
    it("getRoyalty test", async function () {
      engine = await deployProxy(RoyaltyEngineV1, [registry.address], {
        initializer: "initialize",
        from: owner,
      });
      ...
      await mockManifold.setRoyalties(
        manifoldTokenId,
        [manifoldDeployer, random],
        [manifoldBps, randomBps]
      );
      await mockFoundation.setRoyalties(
        foundationTokenId,
        [foundationDeployer, random],
        [foundationBps, randomBps]
      );
      await mockRaribleV1.setRoyalties(
        raribleV1TokenId,
        [raribleV1Deployer, random],
        [raribleV1Bps, randomBps]
      );
      await mockRaribleV2.setRoyalties(
        raribleV2TokenId,
        [raribleV2Deployer, random],
        [raribleV2Bps, randomBps]
      );
      ...
      assert.deepEqual(
        result[1][0],
        web3.utils.toBN((value * manifoldBps) / 10000)
      );
      assert.equal(result[0][1], random);
      assert.deepEqual(
        result[1][1],
        web3.utils.toBN((value * randomBps) / 10000)
      );
      ...
      assert.deepEqual(
        result[1][0],
        web3.utils.toBN((value * foundationBps) / 10000)
      );
      assert.equal(result[0][1], random);
      assert.deepEqual(
        result[1][1],
      ...
      assert.deepEqual(
        result[1][0],
        web3.utils.toBN((value * raribleV1Bps) / 10000)
      );
      assert.equal(result[0][1], random);
      assert.deepEqual(
        result[1][1],
      ...
      assert.deepEqual(
        result[1][0],
        web3.utils.toBN((value * raribleV2Bps) / 10000)
      );
      assert.equal(result[0][1], random);
      assert.deepEqual(
        result[1][1],
      ...
      console.log(
        "CACHE: Payout gas used with override: %s",
        tx.receipt.gasUsed
      );
      ...
      await mockFoundation.setFoundationTreasury(
        mockFoundationTreasury.address
      );
      await truffleAssert.reverts(
        registry.setRoyaltyLookupAddress(
          mockFoundation.address,
      ...
      await truffleAssert.reverts(
        registry.setRoyaltyLookupAddress(
          mockERC1155PresetMinterPauser.address,
          mockManifold.address,
          { from: random }
        )
      );
    ...
    it("invalid royalties test", async function () {
      ...
      await truffleAssert.reverts(
        engine.getRoyaltyView(mockManifold.address, manifoldTokenId, value),
        "Invalid royalty amount"
      );
      await truffleAssert.reverts(
        engine.getRoyaltyView(mockFoundation.address, foundationTokenId, value),
        "Invalid royalty amount"
      );
      await truffleAssert.reverts(
        engine.getRoyaltyView(mockRaribleV1.address, raribleV1TokenId, value),
        "Invalid royalty amount"
      );
      await truffleAssert.reverts(
        engine.getRoyaltyView(mockRaribleV2.address, raribleV2TokenId, value),
        "Invalid royalty amount"
      );
      await truffleAssert.reverts(
        engine.getRoyaltyView(mockEIP2981.address, eip2981TokenId, value),
        "Invalid royalty amount"
      );
      ...
      await mockManifold.setRoyalties(
        manifoldTokenId,
        [defaultDeployer, manifoldDeployer],
        [manifoldBps, manifoldBps]
      );
      await mockFoundation.setRoyalties(
        foundationTokenId,
        [foundationDeployer],
      ...
      await mockRaribleV1.setRoyalties(
        raribleV1TokenId,
        [raribleV1Deployer, raribleV2Deployer],
        [raribleV1Bps, raribleV2Bps]
      );
      await mockRaribleV2.setRoyalties(
        raribleV2TokenId,
        [raribleV2Deployer],
