...
const EIP2981RoyaltyOverrideCloneable = artifacts.require(
  "EIP2981RoyaltyOverrideCloneable"
);
const EIP2981RoyaltyOverrideFactory = artifacts.require(
...
contract("Registry", function ([...accounts]) {
  ...
  describe("Override", function () {
    ...
    beforeEach(async function () {
      ...
      overrideFactory = await EIP2981RoyaltyOverrideFactory.new(
        overrideCloneable.address
      );
    });

    it("override test", async function () {
      await truffleAssert.reverts(
        override.setTokenRoyalties([[1, another1, 1]]),
        "Ownable: caller is not the owner"
      );
      await truffleAssert.reverts(
        override.setDefaultRoyalty([another1, 1]),
        "Ownable: caller is not the owner"
      );
    });
    ...
    it("test", async function () {
      ...
      await truffleAssert.reverts(
        override.setTokenRoyalties([[1, another1, 10000]], { from: admin }),
        "Invalid bps"
      );
      await truffleAssert.reverts(
        override.setDefaultRoyalty([another1, 10000], { from: admin }),
        "Invalid bps"
      );
      ...
      var clone = await EIP2981RoyaltyOverride.at(
        tx.logs[0].args.newEIP2981RoyaltyOverride
      );
      await clone.setTokenRoyalties(
        [
          [3, another3, 300],
          [5, another5, 500],
        ],
      ...
      await clone.setTokenRoyalties(
        [[5, "0x0000000000000000000000000000000000000000", 0]],
        { from: admin }
      );
      result = await engine.getRoyaltyView(mockContract.address, 5, value);
      assert.equal(result[0].length, 1);
