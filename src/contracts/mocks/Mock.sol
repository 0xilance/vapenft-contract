...
contract MockFoundation is IFoundation, IFoundationTreasuryNode, MockRoyalty, ERC165 {
	...
	function getFoundationTreasury() public view override returns (address payable) {
...
contract MockRaribleV1 is IRaribleV1, MockRoyalty, ERC165 {
	...
	function getFeeBps(uint256 tokenId) public view override returns (uint256[] memory) {
		return _bps[tokenId];
	}
...
contract MockRaribleV2 is IRaribleV2, MockRoyalty, ERC165 {
	...
	function getRaribleV2Royalties(uint256 tokenId) public view override returns (IRaribleV2.Part[] memory royalties) {
		royalties = new IRaribleV2.Part[](_receivers[tokenId].length);
		for (uint256 i = 0; i < _receivers[tokenId].length; i++) {
