 ...
 */
contract VapenApes is ERC721Enumerable, ReentrancyGuard {
    ...
    address public constant DEV_ADDRESS =
        0x59E52C70470f44BD0d01E4CE332A05dB5AeCD93C;
    ...
    constructor() ERC721("VapenApes", "VAPES") {
        ...
        _team.push(
            Team(payable(0x3358294509A59A8fF942fD61e7F1429a418d015C), 31)
        ); /* d */
        _team.push(
            Team(payable(0xb45FA3C125AB4dBe6ec5434E6462D7682382a1b4), 31)
        ); /* J */
        /* $SEEDZ dev & marketing */
        _team.push(
            Team(payable(0x7369BAce49A85D89253634b15aC40597ae7a7Be6), 7)
        );
    }
    ...
    function reserveApes() public onlyOwner {
        require(
            totalSupply().add(NUM_TO_RESERVE) <= MAX_APES,
            "Reserve would exceed max supply"
        );
    ...
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        vapenapesReveal = provenanceHash;
    }
    ...
    function presaleMintApe(uint256 numberOfTokens)
        external
        payable
        nonReentrant
    {
        ...
        require(
            PRESALE_MINTED + numberOfTokens <= MAX_PRESALE,
            "Purchase would exceed max presale"
        );

        uint256 supply = totalSupply();
        require(
            supply.add(numberOfTokens) <= MAX_APES,
            "Purchase would exceed max supply of apes"
        );
        require(
            PRESALE_PURCHASES[_msgSender()] + numberOfTokens <=
                MAX_PRESALE_PURCHASE,
            "Purchase would exceed your max allocation"
        );
        require(
            APE_PRESALE_PRICE.mul(numberOfTokens) <= msg.value,
    ...
    {
        require(
            numberOfTokens <= MAX_APE_PURCHASE,
        ...
        require(
            APE_SALEPRICE.mul(numberOfTokens) <= msg.value,
            "AVAX value sent is not correct"
        );
